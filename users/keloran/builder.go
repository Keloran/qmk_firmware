...
func getBoards() ([]Board, error) {
    ...
    if fi.Mode() & os.ModeSymlink != os.ModeSymlink {
        return boards, fmt.Errorf("builder.txt must be a symlink from users folder")
    }
    ...
    if err != nil {
        log.Fatal("you don't have a builder.txt")
    }
    defer f.Close()
    scanner := bufio.NewScanner(f)
    for scanner.Scan() {
        preparse := scanner.Text()
        parts := strings.Split(preparse, ",")
        board := Board{
            Name: parts[0],
            Path: parts[1],
        }
    ...
    if err := scanner.Err(); err != nil {
        ...
        log.Fatalf("scanner failed: %w", err)
    }

    if len(boards) == 0 {
        log.Fatal("you need need to add some boards to the builder.txt, 1 keyboard per line")
    }
...
func main() {
    ...
    if keyboard != "" {
        ...
        if *clean { return } // might just want to clean
        displayHelp()
        return
    }
}
